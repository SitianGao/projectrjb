...
* NLP 的难点
  * 二义性
    * 分词歧义 Lexical Ambiguity
    * 词性歧义 Part-of-speech Ambiguity
    * 结构歧义 Structural Ambiguity
  * 未知语言现象
    * 新名词、人名、地名 New terms, person / place names
    * 新含义 New meaning
    * 新用法 New language usage
* NLP 的应用
  * 机器翻译 Machine Translation
  * 文本生成 Text Generation
  * 信息检索 Information Retrieval
  * 智能问答与对话系统 Question Answering & Dialogue Systems
...
* **文本规范化 Text Normalization**
  ...
  * 分句 Segmenting Sentences
  * 分词 Tokenizing(Segmenting) Words
  * 单词格式规范化 Normalizing Word Formats: 例如 Do, Did, Doing
* **分句 Segmenting Sentences**
  ...
  * "."（句号）——有歧义，它可能是
    ...
    * 缩写 如 Inc.
      * 缩写词典可辅助判断 An Abbreviation dictionary can help
    * 数字 如 1.5
  * 也可通过基于分词结果的规则来判断 Or done by rules based on tokenization
* **基于空格的分词 Word Tokenization: Based on Space**

  * 分词 Tokenizing - 合并 Merging - 排序 Sorting（按计数 By counts）

  * 问题 Issues:

    * 有些单词不能盲目删除标点符号
      * URL / 价格 / 日期 / 电子邮件
    * 多词表达式 Multiword Expression
    * 有些语言不使用空格分隔单词
      * 分字不分词 Single-character Segmentation

  * 子词分词 Subword Tokenization

    * 符号学习器 Token Learning：采用原始语料库并归纳词汇
    * 符号分段器 Token Segmenter：接受测试句子并根据词汇表对其进行分词

    * **BPE 字节对编码 Byte Pair Encoding**
      * 扫描词表（Vocabulary）获得符号集（Individual Character Set）
      * 重复执行：合并当前词表中相邻频率最高的符号，将合并的符号加入到词表中去，把语料库（Corpus）中相邻的该符号对合并起来
      * BPE 符号的特性 BPE Tokens 特性
        * 包含频繁出现的词
        * 发现频繁出现的子词，比如前缀后缀
      * 建议参考：https://zhuanlan.zhihu.com/p/164520154
* **词语规范化 Word Normalization**
  * 大小写统一转换
  * 词根化 Lemmatization：以词根 Lemma 重现这些单词
  * 词干化：通过粗略地切掉词缀将词语缩减为词干 Reduce terms to stems, chopping off affixes crudely
  * **Porter 词干提取算法 Porter stemmer**：基于一系列重写规则
...
* **朴素贝叶斯分类器 Naive Bayes Classifier**
  ...
  * 一些问题：
    ...
    * 停用词 **Stop Word**：有些频繁出现但没有具体含义的词，对分类任务没有什么作用
...
* **精确率、召回率和 F 值 Precision, Recall and F measure**
  ...
  * F 值 F measure：将精确率 Percision 和召回率 Recall 两个指标结合在一起
    $$
...
* **n-gram 语言模型**
  ...
  * 马尔可夫假设 Markov 假设：$P(w_i|w_1,w_2,...w_{i-1}) \approx P(w_i|w_{i-k},...w_{i-1})$
  * Unigram(1-gram), Bigram(2-gram，一阶马尔可夫)...
  * 问题：长句中的从句会把强依赖关系强的词组分开，n-gram 无法发现其关系。
  * $P(w_i|w_{i-1}) = \frac{count(w_{i-1}, w_i)}{w_{i-1}}$
  * 处理细节：在句子之前和句子之后加标识符号再计算。
  * 反应语言学上的分布情况和语法上的知识

* 评估与困惑度 Evaluation & Perplexity
...
* 词干与词义 Lemmas & Sense
  ...
  * 词干 Lemmas：除去词缀
  * 词义 Sense：词语的含义，可能有多重含义 Polysemous
...
* 语义向量 Vector Semantics
  ...
  * 强假设：所有词语的含义都是由其周围的词语决定的
  * 如果两个词语的使用场景完全相同，则它们是同义词
  * 两个方法：
    * 方法1：根据词语在语言学上的分布
    * 方法2：将词语映射到空间中的一个点
...
* 根据词语在语言学上的分布定义语义向量
  ...
  > 将语义定义为空间中基于分布的一个点 Defining meaning as a point in space based on distribution
  >
  > 语言学上的分布指：在词语的语言环境上的分布

  * 如果两个词语很相似，那么它们在向量空间中的距离很近。

* 嵌入 Embedding：将词语映射到一个向量表示（细粒度表示：向量高维度）。
...
* 词向量模型 Word Model
  ...
  * **Tf-idf**：词频-文档频率
    ...
    * PMI 点互信息 Pointwise Mutual Information （另一种方法）

  * **Word2Vec**

    * 稠密表示 Dense Vector
    * **Skip-Gram**：将窗口内同时出现的单词作为正样本对，不出现的作为负样本对，训练过程不是 Triplet Loss，而是将样本对拼接在一起输入模型做二分类（训练正样本和负样本）
      * 因为这个过程不需要人为额外标记标签，所以是自监督的。
      * 将点积表示为概率，用来做逻辑回归训练。最大化目标词和其上下文词汇的相似性，最小化目标词和k个非上下文词汇的相似性。
      * 目标词 Target Word 和上下文词 Content Word 有不同的 Embedding，即两个不同的向量表示，然后将它们加在一起得到一个唯一的词向量
        * 原因：计算方便；同一个词汇同时出现在目标词和上下文中，导致计算结果被多次计算，不符合常理。
      * 一个词向量如何表示多义词汇的问题？首先取决于训练集；新的模型：动态的词向量
      * 为什么要负采样？让词向量在空间中有好的分布。
    * 发现相似关系
      * 基于平行四边形的类比推理，词向量之间可以进行一些运算：
        * apple 之于 tree 如同 grapes 之于 vine：apple 的词向量 - grape 的词向量 = tree 的词向量 - vine 的词向量
...
* 命名实体 Named Entities
  * 人名、地名、组织名
  * 日期、时间、价格
  * BIO 标记方法 Begin-Inside-Outside 将命名实体识别转换为序列标注任务，B 和 I 还要接具体的实体标签例如 PER
    * BIO 的变体：IO、BIOES（E 表示 Ending 结尾，S 表示 Single 单独）

## LXX 机器翻译

* 平行语料 Parallel Corpus：源语句和目标语句 Source Sentence and Target Sentence

* 自由运行模式 Free Running Model：训练时传给下一时刻的是预测词语，测试时传预测词语
  * 难收敛
* 教师强制模式 Teacher-Forcing Mode：训练时传给下一时刻的是真实词语，测试时传预测词语
  * 曝光偏差问题：训练和测试参数一致，但输出不一致
    * 解决方案：以一定概率使用预测词，一定概率使用真实词。
* 多层深度编码器-解码器机器翻译网络 Multi-layer Deep Encoder-decoder machine translation net
...
* **束搜索解码 Beam Search Decoding**：在每个解码位置维护 K（Beam Size，一般设为 5 到 10）个序列（K 个最有可能的部分翻译 K most probable partial translations），每个序列维护一个分数（概率模型） $\sum_{i = 1}^{t}\log P_{LM}(y_i|y_1,...y_{i - 1}, x)$。
  * 不能保证全局最优；
  * 停止准则 Stopping Criterion：特殊的终止符号 <END>，如果前面已经解码出了 <END>，维护的序列个数减少一个；
  * 最后不应该直接选择分数最高的，因为序列长短不一，越长的序列分数越低，可以除以序列长度再挑选最高的。

## 10 注意力机制 Attention

* 带注意力机制的 Seq2Seq 模型 Seq2Seq with Attention:
  * 注意力分数 Attention Scores：将编码器的每一个位置的隐状态与解码器的每一个位置的隐状态做点积，然后对注意力分数做归一化（Softmax）得到注意力分布 Attention Distribution，与编码器的每一个位置的隐状态加权求和得到注意力输出 Attention Output。注意力输出最后与解码器隐状态的输出合并（Concatenate），再计算输出。
    * 要求编码器和解码器隐状态维度一致。
...
* BLEU：双语评估替补 Bilingual Evaluation Understudy
  * N-Gram 模型
  * 简短惩罚 Brevity Penalty，惩罚过短的翻译结果

## L11 成分句法分析

* 成分句法分析 Constituency Parsing
* 成分句法结构 Constituency Syntactic Structure
  * 给每一个词语的词性打标签，单词可以构成不同类别的短语，名词短语可以合并
* 依存句法结构 Dependency Syntactic Structure：扁平化的结构，获取句子之中词语之间的修饰关系，可以获得长距离词语之间的依赖关系。

* 上下文无关文法 CFG Context-Free Grammars

* 概率上下文无关文法 PCFG（可以解决二义性）
