...
async def test_generate_document(agent, sample_profile):
    ...
    raw = await agent.generate_resources(
        topic="线性回归",
...
async def test_difficulty_reflected_in_content(agent):
    """初级和高级的内容应不同"""
    raw_easy = await agent.generate_resources(
        topic="线性回归",
        resource_types=["document"],
        difficulty="初级",
    )
    raw_hard = await agent.generate_resources(
        topic="线性回归",
        resource_types=["document"],
        difficulty="高级",
    )
    easy_content = json.loads(raw_easy)["resources"][0]["content"]
    hard_content = json.loads(raw_hard)["resources"][0]["content"]
...
async def test_no_llm_fallback(agent):
    """无 LLM 时规则化兜底正常工作"""
    raw = await agent.generate_resources(topic="测试", resource_types=["document"])
    data = json.loads(raw)
