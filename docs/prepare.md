...
| 核心开发 | Day 3-7 | 5 个 Agent + Orchestrator | 6 个页面 + Mock 数据 | RAG 管线 + 安全过滤 |
| API+对接 | Day 8-11 | Agent → API 包装 | Mock → 真实 API 对接 | 知识库完善 + 测试 |
| 功能补全 | Day 12-15 | 创新点实现 + 异步任务 | 体验打磨 + 动效 | 题库补充 + 文档初稿 |
| 测试修复 | Day 16-19 | 端到端联调 + Bug 修复 | 响应式适配 + 错误处理 | 测试用例 + 安全验证 |
| 部署文档 | Day 20-24 | 部署 + PPT + 演示 | 演示视频录制 | 文档完善 + 开源标注 |
